...
void proto_to_forward_input(const proto::ForwardInput* pb_forward_input,
  ...
  std::vector<float> logprobs_sum_vec =
      std::vector<float>(pb_forward_input->logprobs_sum_vec().begin(),
                         pb_forward_input->logprobs_sum_vec().end());
  ...
  forward_inputs.logprobs_sum = torch::tensor(
      logprobs_sum_vec,
      torch::dtype(torch::kFloat32).device(torch::kCPU).pinned_memory(true));
  LOG(INFO) << "forward_inputs.logprobs_sum size: "
            << forward_inputs.logprobs_sum.numel();
...
void forward_input_to_proto(const RawForwardInput& inputs,
  ...
  ADD_VECTOR_TO_PROTO(pb_forward_input->mutable_logprobs_sum_vec(),
                      inputs.logprobs_sum_vec);
  LOG(INFO) << "forward_input_to_proto logprobs_sum_vec size: "
            << inputs.logprobs_sum_vec.size();
