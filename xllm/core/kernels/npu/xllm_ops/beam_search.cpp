...
static bool flag = true;  // 是否将输入输出保存为文件
...
void beam_search(const torch::Tensor& logprobs,
  ...
  LOG(INFO) << "beam_search_ops logprobs_ids: " << logprobs;
  LOG(INFO) << "beam_search_ops top_tokens_ids: " << top_tokens;
  LOG(INFO) << "beam_search_ops top_logprobs_ids: " << top_logprobs;
  ...
  LOG(INFO) << "beam_search_ops src_seq_idxes_ids: " << src_seq_idxes;
  LOG(INFO) << "beam_search_ops out_logprobs_ids: " << out_logprobs;
  LOG(INFO) << "beam_search_ops out_tokens_ids: " << out_tokens;
  // 将输入和输出导出为torch.bin文件
  if (flag) {
    torch::save(logprobs, "tensor/logprobs.bin");
    torch::save(top_tokens, "tensor/top_tokens.bin");
    torch::save(top_logprobs, "tensor/top_logprobs.bin");
    torch::save(src_seq_idxes, "tensor/src_seq_idxes.bin");
    torch::save(out_logprobs, "tensor/out_logprobs.bin");
    torch::save(out_tokens, "tensor/out_tokens.bin");
    flag = false;
  }
}
}  // namespace xllm_ops